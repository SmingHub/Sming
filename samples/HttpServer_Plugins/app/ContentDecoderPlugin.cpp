...
bool ContentDecoderPlugin::headersComplete(HttpServerConnection& connection, HttpRequest& request,
...
bool ContentDecoderPlugin::urlComplete(HttpServerConnection& connection, HttpRequest& request, HttpResponse& response)
{
...
bool ContentDecoderPlugin::bodyReceived(HttpServerConnection& connection, HttpRequest& request, char*& data,
										size_t& length)
{
	if(request.headers[HTTP_HEADER_CONTENT_ENCODING] == ENCODING_NAME) {

...
template <class Element> boolean Vector<Element>::contains(const Element& elem) const
{
	for (unsigned int i = 0; i < _size; i++) {
		if (*_data[i] == elem)
			return true;
...
template <class Element> boolean Vector<Element>::isEmpty() const
{
	return _size == 0;
};
...
template <class Element> boolean Vector<Element>::removeElement(const Element& obj)
{
	for (unsigned int i = 0; i < _size; i++) {
		if (*_data[i] == obj) {
			removeElementAt(i);

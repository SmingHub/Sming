...
void SDCard_begin(uint8 PIN_CARD_SS, uint8 byteOrder, uint32 freqLimit)
{
	...
	if (freqLimit < 4000000) SPI_init_freq = freqLimit; /* Test if the limit frequency is higher that the init frequency */
	else SPI_init_freq = 4000000;						  /* It is useful to debug at a lower speed so the logic analyser can catch everything */

	if (freqLimit <= 40000000) SPI_freqLimit = freqLimit; /* Test to see if frequency is too high */
	else SPI_freqLimit = 40000000;							 /* Some SD cards struggle at 40MHz */

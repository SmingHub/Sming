...
void Sparkfun_L6470::configSyncPin(byte pinFunc, byte syncSteps)
{
...
void Sparkfun_L6470::configStepMode(byte stepMode)
{
...
float Sparkfun_L6470::getMaxSpeed()
{
...
void Sparkfun_L6470::setMinSpeed(float stepsPerSecond)
{
  // We need to convert the floating point stepsPerSecond into a value that
  //  the dSPIN can understand. Fortunately, we have a function to do that.
...
float Sparkfun_L6470::getMinSpeed()
{
...
//  mode. 
void Sparkfun_L6470::setFullSpeed(float stepsPerSecond)
{
  unsigned long integerSpeed = FSCalc(stepsPerSecond);
  setParam(FS_SPD, integerSpeed);
}

float Sparkfun_L6470::getFullSpeed()
{
...
void Sparkfun_L6470::setPWMFreq(uint16_t divisor, uint16_t multiplier)
{
...
uint8_t Sparkfun_L6470::getPWMFreqDivisor()
{
  return (uint8_t) (getParam(CONFIG) & 0xE000);
}

uint8_t Sparkfun_L6470::getPWMFreqMultiplier()
{
  return (uint8_t) (getParam(CONFIG) & 0x1C00);
}

// Slew rate of the output in V/us. Can be 180, 290, or 530.
void Sparkfun_L6470::setSlewRate(uint16_t slewRate)
{
  unsigned long configVal = getParam(CONFIG);
...
uint8_t Sparkfun_L6470::getSlewRate()
{
  return (uint8_t) (getParam(CONFIG) & 0x0300);
}

// Single bit- do we shutdown the drivers on overcurrent or not?
void Sparkfun_L6470::setOCShutdown(uint8_t OCShutdown)
{
  unsigned long configVal = getParam(CONFIG);
...
uint8_t Sparkfun_L6470::getOCShutdown()
{
  return (uint8_t) (getParam(CONFIG) & 0x0080);
}

// Enable motor voltage compensation? Not at all straightforward- check out
//  p34 of the datasheet.
void Sparkfun_L6470::setVoltageComp(uint8_t vsCompMode)
{
  unsigned long configVal = getParam(CONFIG);
...
uint8_t Sparkfun_L6470::getVoltageComp()
{
  return (uint8_t) (getParam(CONFIG) & 0x0020);
}

// The switch input can either hard-stop the driver _or_ activate an interrupt.
//  This bit allows you to select what it does.
void Sparkfun_L6470::setSwitchMode(uint8_t switchMode)
{
  unsigned long configVal = getParam(CONFIG);
...
uint8_t Sparkfun_L6470::getSwitchMode()
{
  return (uint8_t) (getParam(CONFIG) & 0x0100);
}
...
void Sparkfun_L6470::setOscMode(uint8_t oscillatorMode)
{
  unsigned long configVal = getParam(CONFIG);
...
uint8_t Sparkfun_L6470::getOscMode()
{
  return (uint8_t) (getParam(CONFIG) & 0x000F);
}
...
void Sparkfun_L6470::setAccKVAL(byte kvalInput)
{
  setParam(KVAL_ACC, kvalInput);
}

byte Sparkfun_L6470::getAccKVAL()
{
  return (byte) getParam(KVAL_ACC);
}

void Sparkfun_L6470::setDecKVAL(byte kvalInput)
{
  setParam(KVAL_DEC, kvalInput);
}

byte Sparkfun_L6470::getDecKVAL()
{
  return (byte) getParam(KVAL_DEC);
}

void Sparkfun_L6470::setRunKVAL(byte kvalInput)
{
  setParam(KVAL_RUN, kvalInput);
}

byte Sparkfun_L6470::getRunKVAL()
{
  return (byte) getParam(KVAL_RUN);
}

void Sparkfun_L6470::setHoldKVAL(byte kvalInput)
{
  setParam(KVAL_HOLD, kvalInput);
}

byte Sparkfun_L6470::getHoldKVAL()
{
...
void Sparkfun_L6470::setLoSpdOpt(boolean enable)
{
...
boolean Sparkfun_L6470::getLoSpdOpt()
{

...
void init()
{
    ...
    auto binaryData = String(gifData);
    gif.open(reinterpret_cast<uint8_t*>(const_cast<char*>(binaryData.c_str())), gifData.length(), draw);

    playTimer.initializeMs(100, nextFrame).start();
}

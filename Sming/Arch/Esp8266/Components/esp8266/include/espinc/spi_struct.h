...
typedef struct {
    ...
    union ctrl_t {
        struct {
            ...
            uint32_t share_bus: 1;
            uint32_t hold_mode: 1;
            uint32_t enable_ahb: 1;
            uint32_t sst_aai: 1;
            uint32_t res_and_res: 1;
            uint32_t fread_quad:         1;                 /*In the read operations read-data phase apply 4 signals. 1: enable 0: disable.*/
            uint32_t wp_reg: 1; /* Idle state for IO3 in 2-wire modes. 0: Idle LOW; 1: Idle high */
            uint32_t two_byte_status_en: 1;
    ...
    union user_t {
        struct {
            uint32_t duplex:            1;
            uint32_t ahb_usr_command_4byte: 1;

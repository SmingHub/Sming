...
namespace sha2
{
template <typename T> __forceinline T CH(T e, T f, T g)
{
	return ((f ^ g) & e) ^ g;
}
 ...
 */
template <typename T, int a0, int b0, int c0, int a1, int b1, int c1> class BlockShift
{
public:
 ...
 */
template <typename T, int a0, int b0, int c0, int a1, int b1, int c1> class StateShift
{
public:
	static __forceinline T s0(T x)
	{
 ...
 */
template <class R, unsigned len, typename T> void extend(T w[], const uint8_t block[])
{
	Range<16>::decode(w, block);
	for(unsigned i = 16; i < len; ++i) {
		w[i] = R::s1(w[i - 2]) + w[i - 7] + R::s0(w[i - 15]) + w[i - 16];
	}
}
 ...
 */
template <class R, typename T> __forceinline void step(T a, T b, T c, T& d, T e, T f, T g, T& h, T w, T k)
{
	T temp1 = h + R::s1(e) + CH(e, f, g) + k + w;
	T temp2 = R::s0(a) + MAJ(a, b, c);
	d += temp1;
	h = temp1 + temp2;
}
...
{
	step<R>(a, b, c, d, e, f, g, h, w[i + 0], k[i + 0]);
	step<R>(h, a, b, c, d, e, f, g, w[i + 1], k[i + 1]);
	step<R>(g, h, a, b, c, d, e, f, w[i + 2], k[i + 2]);
	step<R>(f, g, h, a, b, c, d, e, w[i + 3], k[i + 3]);
	step<R>(e, f, g, h, a, b, c, d, w[i + 4], k[i + 4]);
	step<R>(d, e, f, g, h, a, b, c, w[i + 5], k[i + 5]);
	step<R>(c, d, e, f, g, h, a, b, w[i + 6], k[i + 6]);
	step<R>(b, c, d, e, f, g, h, a, w[i + 7], k[i + 7]);
}

/**
 * Process a block of data
 */
template <unsigned len, class BlockShift, class StateShift, typename T>
__forceinline void process(T state[], const uint8_t block[], const T k[])
{
	T w[len];
	extend<BlockShift, len>(w, block);
